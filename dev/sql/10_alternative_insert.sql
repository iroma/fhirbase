...
WITH vals AS (
  SELECT fhir.underscore(a.key) as key,
    ...
    WHERE fhir.underscore(a.key) = dc.column_name::text
        AND dc.table_name = _table_name
        AND dc.table_schema = 'fhir'
        AND dc.data_type IS NOT NULL
), key_vals AS (
...
select 'insert into '
   || 'fhir.' || _table_name
   || ' (' || string_agg(fhir.underscore(key),',') || ') '
...
select meta.eval_ddl(
      ...
      || E'\n'
      || 'SELECT resource_id, uuid, path, parent_id, value from patient '
      || E'\nUNION ALL'
      || E'\n'
...
SELECT
  meta.eval_insert(
    build_insert_satement(
      fhir.table_name(path)::text,
      value,
      id::text,
      parent_id::text,
      resource_id::text
